    ...
    <>
      ...
      <section className="decision-grid">
        ...
        <DemandOutlookPanel
          ...
          forecastTooltip={forecast.source === "ML model" ? `Product ID ${product.id} is used to retrieve product attributes, actual sales momentum, discount/price, store coverage, restock signal, category benchmark, and Google Trends. The XGBoost model outputs next 4w, 8w, and 12w demand; the chart displays the 12w output. Test accuracy: ${forecast.modelPredictions?.testAccuracyPct ?? "N/A"}%.` : `Projected units for ${forecast.window}. Formula: scaled analog/category lifecycle forecast (${formatK(forecast.baseTotal)} units) x Google Trends adjustment (${trendMultiplier}). Trend uses weighted ${trendKeys} search interest in ${forecast.window} versus the previous 12 lifecycle weeks, capped between 0.85x and 1.15x.`}
        ...
        <RecommendationPanel
          ...
          metrics={[
            ...
            { label: "Key driver", value: forecast.source === "ML model" ? "Sales momentum + operational signals" : forecast.matchedCoverageWeeks >= 6 ? "Matched analog tail demand" : "Category tail fallback" },
          ]}
